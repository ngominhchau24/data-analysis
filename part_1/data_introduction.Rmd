---
title: "Giới thiệu và Kiểm tra Chất lượng Dữ liệu Coffee NIR"
...
```{r load-data}
# Đọc dữ liệu
data <- read.csv("../coffee_nirs.csv", sep = ";", row.names = 1, stringsAsFactors = FALSE)
...
cat("- CGA:", class(data$CGA), "\n")
cat("- Localisation:", class(data$Localisation), "\n")
cat("- S1:", class(data$S1), "\n")
```

Bộ dữ liệu có **`r nrow(data)`** mẫu và **`r ncol(data)`** biến.

# 2. Các Nhóm Biến trong Dữ liệu

Dữ liệu Coffee NIR được chia thành 3 nhóm biến chính:

## 2.1 Biến Hóa lý (Chemical-Physical Variables)
...
## 2.2 Biến Vị trí địa lý (Location)
...
## 2.3 Biến NIR (Near-Infrared Spectroscopy)

```{r nir-vars}
# Các biến NIR (S1 đến S1050)
nir_vars <- grep("^S[0-9]+$", names(data), value = TRUE)
...
set.seed(123)
sample_indices <- sample(1:nrow(data), 10)

# Chuyển dữ liệu sang dạng long format để vẽ
nir_data_long <- data[sample_indices, nir_vars] %>%
...
# 3. Kiểm tra Dữ liệu Thiếu (Missing Data)
...
cat("Tổng số quan sát thiếu:", sum(is.na(data)), "\n")
cat("Tỷ lệ dữ liệu thiếu tổng thể:", round(mean(is.na(data)) * 100, 2), "%\n")
```

## 3.1 Phân tích chi tiết Missing Data
...
## 3.2 Trực quan hóa Missing Data
...
if(sum(is.na(data[, chemical_vars])) > 0) {
  gg_miss_var(data[, chemical_vars]) +
    labs(title = "Số lượng missing values theo biến hóa lý")
}
```

## 3.3 Nhận xét về Missing Data
...
# 4. Kiểm tra Giá trị Lạ (Outliers)

## 4.1 Outliers trong Biến Hóa lý

```{r outliers-chemical}
# Chuẩn hóa dữ liệu hóa lý để so sánh
data_chemical <- data[, chemical_vars]
...
### 4.1.1 Phân tích chi tiết Outliers
...
for(var in chemical_vars) {
  result <- detect_outliers(data[[var]])
...
nir_outlier_counts <- sapply(data[, sample_nir_vars], function(x) {
...
data_nir_sample <- data[, sample_nir_vars[1:9]] %>%
  pivot_longer(everything(), names_to = "variable", values_to = "value")
...
## 4.3 Phân tích Multivariate Outliers

```{r multivariate-outliers}
# Sử dụng Mahalanobis distance cho biến hóa lý
data_chem_complete <- data[complete.cases(data[, chemical_vars]), chemical_vars]
...
# 5. Tại sao cần phát hiện và xử lý Outliers?

## 5.1 Lý do cần kiểm tra Outliers
...
cat("- Số mẫu:", nrow(data), "\n")
cat("- Số biến tổng:", ncol(data), "\n")
