...
if(summary(anova_pc1)[[1]][1, "Pr(>F)"] < 0.05) {
  cat("Có sự khác biệt có ý nghĩa giữa các Location trên PC1\n\n")
...
- PC1 và PC2 giải thích `r round(cumvar[2], 2)`% phương sai tổng thể
...
- `r if(ari > 0.5) "Kết quả clustering phù hợp tốt với phân loại Location thực tế" else if(ari > 0.3) "Kết quả clustering phù hợp trung bình với phân loại Location thực tế" else "Kết quả clustering khác biệt đáng kể so với phân loại Location thực tế"`
