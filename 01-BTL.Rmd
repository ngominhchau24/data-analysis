...
```{r load-data}
# Data already loaded in index.Rmd global-setup chunk
...
cat(sprintf("- CGA: %s\n- Localisation: %s\n- S1: %s\n",
            class(coffee_data$CGA),
            class(coffee_data$Localisation),
            class(coffee_data$S1)))
```
...
```{r chemical-vars}
# Variable groups already defined in index.Rmd global-setup
...
cat(sprintf("Tổng số biến có dữ liệu thiếu: %d\nTổng số quan sát thiếu: %d\nTỷ lệ dữ liệu thiếu tổng thể: %.2f%%\n",
            sum(miss_summary$n_miss > 0),
            sum(is.na(coffee_data)),
            round(mean(is.na(coffee_data)) * 100, 2)))
```
...
if(nrow(miss_nir) > 0) {
  ...
  cat("Không có dữ liệu thiếu trong các biến NIR\n")
}
```
...
- Tổng số ô dữ liệu: `r format(total_cells, big.mark = ",")`
- Số ô thiếu dữ liệu: `r format(missing_cells, big.mark = ",")`
- Tỷ lệ thiếu: `r round(missing_pct, 3)`%

`r if(missing_pct < 1) "Bộ dữ liệu có chất lượng tốt với tỷ lệ thiếu dữ liệu rất thấp." else if(missing_pct < 5) "Bộ dữ liệu có chất lượng khá tốt, cần xem xét xử lý missing data." else "Bộ dữ liệu có nhiều giá trị thiếu, cần xử lý cẩn thận."`
...
**Trong dữ liệu này, ta quan tâm đến:**

1. **Univariate outliers**: Giá trị bất thường trong từng biến riêng lẻ
   - Phát hiện bằng IQR method, Z-score
   - Ví dụ: Hàm lượng Cafeine quá cao hoặc quá thấp

2. **Multivariate outliers**: Tổ hợp giá trị bất thường giữa nhiều biến
   - Phát hiện bằng Mahalanobis distance, PCA
   - Ví dụ: Mẫu có tổ hợp CGA-Cafeine bất thường

3. **Spectral outliers**: Phổ NIR bất thường
   - Phát hiện bằng PCA score plots, Hotelling T2
   - Quan trọng để đảm bảo chất lượng phổ
...
- Số mẫu: `r nrow(coffee_data)`
- Số biến tổng: `r ncol(coffee_data)`
  + Biến hóa lý: `r length(chemical_vars)`
  + Biến vị trí: 1
  + Biến NIR: `r length(nir_vars)`

**Chất lượng dữ liệu:**

- Tỷ lệ dữ liệu thiếu: `r round(missing_pct, 3)`%
- Số vị trí địa lý: `r nrow(location_summary)`

**Outliers:**

- Các biến hóa lý có chứa outliers (xem bảng phân tích ở trên)
- Cần xem xét kỹ trước khi quyết định xử lý

**Khuyến nghị:**

1. Bộ dữ liệu có chất lượng tốt, ít missing data
2. Cần xử lý outliers một cách cẩn thận trước khi xây dựng mô hình
3. Dữ liệu NIR cần được tiền xử lý (smoothing, normalization) ở bước tiếp theo
4. Xem xét phân tích theo nhóm Localisation nếu thấy sự khác biệt đáng kể
