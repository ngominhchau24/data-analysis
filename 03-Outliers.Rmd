...
if(outlier_pct < 1) {
  ...
  cat("   - Xem xét loại bỏ các outliers có cả T² và Q cao\n")
...
if(nrow(all_outliers) > 0) {
  cat("   - High T² only:", sum(all_outliers$Outlier_Type == "High T² only"),
...
```{r outlier-conclusion}
cat("### Tóm tắt phát hiện Outliers:\n\n")
cat("- **Phương pháp sử dụng:**\n")
cat("  + Hotelling T² statistic (khoảng cách trong không gian PC)\n")
...
cat("  + Outliers theo T²:", length(t2_outliers), "\n")
cat("  + Outliers theo Q:", length(q_outliers), "\n")
cat("  + Tổng outliers (T² hoặc Q):", total_outliers, "\n\n")
