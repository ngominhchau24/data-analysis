...
for(var in chemical_vars) {
  ...
  pls_cv <- plsr(Y ~ ., data = pls_data_clean, validation = "CV",
...
for(i in 1:length(chemical_vars)) {
  ...
  # R²_CV - Calculate manually from RMSECV (same approach as PLS)
  rmsecv_value <- rmsep_obj$val[1, 1, ncomp + 1]
  var_y <- var(Y_train)
  pcr_performance$R2_CV[i] <- 1 - (rmsecv_value^2 / var_y)
}
...
### Mô hình tốt nhất cho từng chỉ tiêu:
...
- R² của PLS đều âm (-0.12 đến -0.43), cho thấy model không fit tốt với dữ liệu này
- R² của PCR tốt hơn (gần 0 hoặc dương nhỏ), nhưng vẫn ở mức thấp
- Cả hai mô hình đều cho hiệu suất dự đoán khá hạn chế với bộ dữ liệu này

**Nguyên nhân có thể:**
- Dữ liệu có nhiều biến động (variation) lớn giữa các mẫu
- Mối quan hệ giữa phổ NIR và các chỉ tiêu hóa lý có thể không tuyến tính
...
**VIP Analysis:**
- VIP scores giúp xác định các bước sóng quan trọng trong dự đoán
- Có thể sử dụng để feature selection hoặc hiểu mối quan hệ hóa học

**Ý nghĩa thực tiễn:**

- Phương pháp NIR spectroscopy kết hợp PLS/PCR có tiềm năng cho dự đoán nhanh
- Tuy nhiên, với bộ dữ liệu hiện tại, độ chính xác còn hạn chế
- **Khuyến nghị:**
  - Cần thu thập thêm dữ liệu để cải thiện model
...
- Trong trạng thái hiện tại, model chưa đủ tin cậy để ứng dụng thực tế
