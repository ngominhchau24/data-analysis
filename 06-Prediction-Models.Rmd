...
for(i in 1:length(chemical_vars)) {
  ...
  # R2() does not have 'estimate' parameter, it returns all estimates
  r2_cv <- R2(model)
  pls_performance$R2_CV[i] <- r2_cv$val[1, 1, ncomp + 1]

  # RMSEP from training
  rmsep_train <- RMSEP(model, estimate = "train")
  pls_performance$RMSEP[i] <- rmsep_train$val[1, 1, ncomp + 1]

  # R² from training - Use fitted values instead of predict
  # This is safer and avoids array extraction issues
  train_data <- pls_train_data[[var]]
  Y_train <- train_data$Y

  # Get fitted values for the optimal number of components
