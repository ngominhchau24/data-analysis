...
for(i in 1:length(chemical_vars)) {
  ...
  # R² from training - calculate manually
  fitted_vals <- as.vector(fitted(model)[, , ncomp])
  actual_vals <- coffee_data[[var]]
  # Remove NA values if any
  valid_idx <- complete.cases(actual_vals, fitted_vals)
  actual_clean <- actual_vals[valid_idx]
  fitted_clean <- fitted_vals[valid_idx]
  ss_res <- sum((actual_clean - fitted_clean)^2)
  ss_tot <- sum((actual_clean - mean(actual_clean))^2)
