...
for(i in 1:length(chemical_vars)) {
  ...
  # Convert to integer to avoid named vector issues
  ncomp <- as.integer(optimal_ncomp[var])
  ...
  # Predict using the model with the full training data frame
  # predict.mvr returns 3D array [samples, 1, ncomp], need to extract correctly
...
for(i in 1:length(chemical_vars)) {
  ...
  # Use only complete cases for prediction (matching training data)
  X_clean <- X[complete_idx, ]

  # Lấy giá trị dự đoán using cleaned data
  # Convert matrix to data frame for predict() function
  newdata <- as.data.frame(X_clean)
  # predict.mvr returns 3D array [samples, 1, ncomp], extract correctly
  pred_result <- predict(model, ncomp = ncomp, newdata = newdata)
  predictions <- as.vector(pred_result[, 1, ncomp])
...
for(i in 1:length(chemical_vars)) {
  ...
  # Tính residuals using cleaned data
  # Convert matrix to data frame for predict() function
  newdata <- as.data.frame(X_clean)
  # predict.mvr returns 3D array [samples, 1, ncomp], extract correctly
  pred_result <- predict(model, ncomp = ncomp, newdata = newdata)
