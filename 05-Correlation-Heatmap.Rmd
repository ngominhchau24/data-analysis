...
cat("\n**Ý nghĩa:**\n")
cat("- PC có tương quan cao với biến hóa lý cho thấy thành phần chính đó\n")
cat("  chứa thông tin quan trọng để dự đoán biến đó\n")
cat("- Có thể sử dụng PC scores làm biến độc lập trong mô hình hồi quy\n")
```
...
cat("Xem biểu đồ heatmap phía dưới để so sánh pattern tương quan giữa các vị trí địa lý.\n\n")
...
for(loc in unique_locations) {
  ...
  if(length(loc_idx) > 5) {  # Chỉ phân tích nếu có đủ mẫu
    ...
    # Note: Detailed tables hidden to avoid clutter - see heatmap visualization instead
  }
}
```
...
cat("\n**4. Khuyến nghị:**\n")
cat("- Sử dụng wavelengths có tương quan cao để xây dựng mô hình đơn giản\n")
cat("- Cân nhắc sử dụng PC scores thay vì toàn bộ NIR để giảm chiều dữ liệu\n")
cat("- Lưu ý sự khác biệt tương quan giữa các Location khi xây dựng mô hình\n")
cat("- Các biến hóa lý có tương quan cao với nhau có thể gây multicollinearity\n")
```
